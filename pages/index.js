...
export default function Home({ listCategory, mostFavoriteItem, rankingCollection }) {
  ...
  const { collectionRanking } = useRankingCollection(
    `start_time=${rangeTime[0]}&end_time=${rangeTime[1]}&category_id=${categoryId}`,
    rankingCollection
  );

  const listFeatureCategory = listCategory.map((item, index) => {
    return <FeatureCategory key={index} category={item} />;
  });
  ...
  return (
    <>
      <div className={styles.content}>
        ...
        <FeatureTrending
          rankingCollection={collectionRanking}
...
export async function getStaticProps() {
  ...
  const rangeTime = [moment().subtract(7, "day").format(DATE_TIME), moment().format(DATE_TIME)];
  const rankingCollection = await getRankingCollection(
    `start_time=${rangeTime[0]}&end_time=${rangeTime[1]}`
  );
  return {
    props: {
      listCategory: listCategory || [],
